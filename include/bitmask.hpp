...
struct mask_from_max_element {
	static constexpr underlying_type_t<T> max_element_value_ = static_cast<underlying_type_t<T>>(MaxElement);
	...
	static constexpr underlying_type_t<T> value = max_element_value_ ? ((max_element_value_ - 1) << 1) + 1 : 0;
};
...
public:
	...
	constexpr bitmask(std::nullptr_t) noexcept
		: m_bits {0} { }
...
operator&(T l, const bitmask<T> &r) noexcept {
	return r & l;
}
...
operator!=(const bitmask<T> &l, const bitmask<T> &r) noexcept {
...
template <class T>
inline constexpr bool
operator!=(const bitmask<T> &l, T r) noexcept {
...
template <class T>
inline constexpr bool
operator!=(const bitmask_detail::underlying_type_t<T> &l, const bitmask<T> &r) noexcept {
	return l != r.bits();
}
...
template <class T>
inline constexpr bool
operator!=(const bitmask<T> &l, const bitmask_detail::underlying_type_t<T> &r) noexcept {
	return l.bits() != r;
}
...
bits(const bitmask<T> &bm) noexcept {
	return bm.bits();
}
...
#define BITMASK_DETAIL_DEFINE_OPS(value_type)                                                      \
	inline constexpr bitmask::bitmask<value_type> operator&(value_type l, value_type r) noexcept { \
		return bitmask::bitmask<value_type> {l} & r;                                               \
	}                                                                                              \
	inline constexpr bitmask::bitmask<value_type> operator|(value_type l, value_type r) noexcept { \
		return bitmask::bitmask<value_type> {l} | r;                                               \
	}                                                                                              \
	...
	inline constexpr bitmask::bitmask<value_type> operator~(value_type op) noexcept {              \
		return ~bitmask::bitmask<value_type> {op};                                                 \
	}                                                                                              \
	inline constexpr bitmask::bitmask<value_type>::underlying_type bits(value_type op) noexcept {  \
		return bitmask::bitmask<value_type> {op}.bits();                                           \
	}                                                                                              \
	...
	class BITMASK_MAKE_UNIQUE_NAME(_disable_unused_function_warnings_) {                           \
		static constexpr int _unused() noexcept {                                                  \
			return bitmask::bitmask_detail::disable_unused_function_warnings<value_type>(), 0;     \
		}                                                                                          \
	};                                                                                             \
	}
