// clang-format off
	...
	*/
	...
	// #define RFID_READER_TYPE_PN5180       // use PN5180 via SPI
		...
		// #define RFID_READER_TYPE_MFRC522_I2C
		...
		constexpr auto rfidPassword = std::to_array<std::array<uint8_t, 4>>({
			{{0x0F, 0x0F, 0x0F, 0x0F}},
			{{0xAB, 0xEF, 0x00, 0x00}}
		});
	#endif

	#if defined(RFID_READER_TYPE_MFRC522) || defined(RFID_READER_TYPE_PN532) || defined(RFID_READER_TYPE_PN5180)
		#define RFID_READER_ENABLE 1
	#endif
	...
	*/
	...
	// (optinal) Neopixel
		...
		#define NUM_CONTROL_LEDS		 3				// optional control leds (https://forum.espuino.de/t/statische-ws2812-leds/1703)
                #define CONTROL_LEDS_COLORS		{CRGB::Green, CRGB::Red, CRGB::SteelBlue}		// Colors for the control LEDs. Make sure it lists at least NUM_CONTROL_LEDS colors, e.g. for three control LEDs define: CONTROL_LEDS_COLORS {CRGB::Yellow, CRGB::Blue, 0xFFFFFF} (predefined colors: http://fastled.io/docs/3.1/struct_c_r_g_b.html)
	...
	#elif (HAL == 97)
		#include "settings-ttgo_t8_sdmmc.h"            // Pre-configured settings for Lolin D32 (non-pro) with SDMMC + port-expander (https://forum.espuino.de/t/espuino-minid32-pro-lolin-d32-pro-mit-sd-mmc-und-port-expander-smd/866)
	#elif (HAL == 99)
