...
bool fileValid(const char *_fileItem) {
	...
	constexpr auto audioFileSufix = std::to_array<const char*>({
		".mp3",
		".aac",
		".m4a",
		".wav",
	...
	// check for streams
	// TODO: check here for streams to support m3u playlists with mixed content
	...
	if (extended) {
		...
		while (f.available()) {
			...
			if (!line.startsWith("#")) {
				...
				line.trim();
				// save string
				if (!SdCard_allocAndSave(playlist.get(), line)) {
					return std::nullopt;
				}
			}
		}
...
std::optional<std::unique_ptr<Playlist>> SdCard_ReturnPlaylist(const char *fileName, const uint32_t _playMode) {
	...
	if (!fileOrDirectory.isDirectory()) {
		if (!SdCard_allocAndSave(playlist.get(), fileOrDirectory.path())) {
			return std::nullopt;
		}
		return playlist;
	}
	...
	while (true) {
		...
		if (fileValid(name.c_str())) {
			...
			if (!SdCard_allocAndSave(playlist.get(), name)) {
				return std::nullopt;
			}
		}
	}
