...
struct CardActions {
	enum EnumValues : uint16_t {
		...
		BUSY = 10, // Used if playlist is created
		...
		max = CMD_RESTARTSYSTEM + 1
	};
	using type = std::underlying_type_t<EnumValues>;

	EnumValues t_;
	CardActions(EnumValues t)
		: t_(t) { }
	CardActions(const type t) { t_ = operator=(t); }
	CardActions() = default;

	CardActions &operator=(const CardActions &other) {
		t_ = other.t_;
		return *this;
	}
	CardActions &operator=(const type &i) {
		if (i < EnumValues::max) {
			t_ = static_cast<EnumValues>(i);
		}
		return *this;
	}

	operator EnumValues() const { return t_; }
