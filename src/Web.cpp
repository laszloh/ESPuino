...
bool JSONToSettings(JsonObject doc) {
	...
	if (doc.containsKey("bluetooth")) {
		...
		rfid::resetOldRfid(); // Set old rfid-id to crap in order to allow to re-apply a new assigned rfid-tag exactly once
#endif
...
static void settingsToJSON(JsonObject obj, const String section) {
	if ((section == "") || (section == "current")) {
		...
		curObj["rfidTagId"] = rfid::getCurrentRfidTagId().toDezimalString();
	}
...
void Web_SendWebsocketData(uint32_t client, uint8_t code) {
	...
	if (code == 1) {
		...
		object["rfidId"] = rfid::getCurrentRfidTagId().toDezimalString();
	} else if (code == 20) {
		object["pong"] = "pong";
