...
// custom converter for our types, so that we can write doc["x"] = variable
// 		see: https://arduinojson.org/news/2021/05/04/version-6-18-0/
void convertToJson(const CardActions &action, JsonVariant doc) {
	doc.set((int) action);
}
bool canConvertFromJson(JsonVariantConst src, const CardActions &) {
	return src.as<CardActions::type>() < CardActions::max;
}
void convertFromJson(JsonVariantConst src, CardActions &dst) {
	dst = static_cast<CardActions>(src.as<CardActions::type>());
}
...
bool JSONToSettings(JsonObject doc) {
	...
	if (doc.containsKey("bluetooth")) {
		...
		if (doc["controls"].containsKey("action")) {
			CardActions cmd = doc["controls"]["action"].as<uint8_t>();
			Cmd_Action(cmd);
		}
...
void explorerHandleDeleteRequest(AsyncWebServerRequest *request) {
	...
	if (request->hasParam("path")) {
		...
		if (gFSystem.exists(filePath)) {
			...
			Cmd_Action(CardActions::CMD_STOP);
			file = gFSystem.open(filePath);
			if (file.isDirectory()) {
