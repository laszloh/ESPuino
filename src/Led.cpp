...
AnimationReturnType Animation_PlaylistProgress(const bool startNewAnimation, CRGBSet &leds) {
	...
	if constexpr (NUM_INDICATOR_LEDS >= 4) {
		if (AudioPlayer_GetTrackCount() > 1 && gPlayProperties.currentTrackNumber < AudioPlayer_GetTrackCount()) {
			const uint32_t ledValue = std::clamp<uint32_t>(map(gPlayProperties.currentTrackNumber, 0, AudioPlayer_GetTrackCount() - 1, 0, leds.size() * DIMMABLE_STATES), 0, leds.size() * DIMMABLE_STATES);
			const uint8_t fullLeds = ledValue / DIMMABLE_STATES;
			const uint8_t lastLed = ledValue % DIMMABLE_STATES;
