	...
	switch (mod) {
		...
		case CMD_SLEEP_AFTER_5_TRACKS: {
			...
			if (gPlayProperties.sleepAfter5Tracks) {
				...
				if (gPlayProperties.currentTrackNumber + 5 > gPlayProperties.playlist->size()) { // If currentTrack + 5 exceeds number of tracks in playlist, sleep after end of playlist
					gPlayProperties.sleepAfterPlaylist = true;
#ifdef MQTT_ENABLE
					publishMqtt(topicSleepTimerState, "EOP", false);
#endif
